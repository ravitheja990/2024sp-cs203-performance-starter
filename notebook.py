...
#        t = pd.DataFrame(["function","reps","IC","CPI","CT","ET","MHz"])
        r = pd.DataFrame( [df['function'][0],int(f.split(".")[0]),df['IC'].sum(),df['CPI'].mean(),df['CT'].mean(),df['ET'].sum(),df['MHz'].mean()]).T
#        all = pd.DataFrame([all,r])
        all = pd.concat([all, r], ignore_index=True)
#    all.set_index(["function","reps","IC","CPI","CT","ET","MHz"])
    all.columns=["function","reps","IC","CPI","CT","ET","MHz"]
    return all
...
def nbturnin(notebook=None, out=None):
    ...
    for cell_index, cell in reversed(list(enumerate(nb.cells))): # in reverse so we can delete in place.
        ...
        if cell.metadata.get("cse142.is_answer_start", False): 
            in_answer_section -= 1  # the sign is backward because we process the notebook in reverse.
        if cell.metadata.get("cse142.is_answer_end", False):
            in_answer_section += 1
            
        if cell.metadata.get('cse142.is_response', False) or in_answer_section > 0:
            pass
        else:
...
print("Done loading notebook.py! We're good to go!");